...
三条设计红线：**判别流水线任何环节不得依赖人工判定**（需要人判别的功能等于没有）；**成本有硬上限**（预算制，不随时长线性增长）；**新增场所不改核心代码**（平台成立判据）。

**宣传口径**：当前算法是"可配置事件摄像头 + 场所判别头"——不宣称自动理解所有场景、零误报或人类级语义理解；只报告实测数据，未测项标"待回填/设计态"。

当前验证状态：初始化探针头离线精度 **98.15%**（162 张权威 Drone-vs-Bird 样本，证据 `web/jepa_probe_init.json`：acc=0.9815, n_train=162, dim=768，自前身仓实测继承）；WHEP 信令已用本地 MediaMTX v1.20.0 + H.264 测试流完成端到端验证；**46 例单元测试 + GitHub Actions CI 全绿**（含双模式端到端验收与核心源码洁净度元测试）。真机端到端帧率/延迟实测**待回填**。
...
| 证据事件 | `sc.evidence/v1` 版本化事件信封：检测置信与判别置信分立、布防状态、模型/探针版本可追溯，告警附裁剪帧——从"帧"到"有证据的事件"，供机器消费与审计回放 |
...
```bash
node --test tests/core.test.mjs     # 46 例单测（node:test，零依赖）
...
| 单元测试 / CI | ✅ 46 例全绿（含双模式验收 + 洁净度元测试），node 20/22 矩阵 |
...
里程碑 M1（判别自动化）→ M1.5（独立建仓+配置治理）→ **M1.6（平台化收敛：核心去领域化 + 双模式验收 + 证据事件，本轮）** → M2（vus 桥仲裁回灌）→ M3（时空规则引擎+开放集+油库烟火包）→ M4（多相机调度+健康监控）→ M5（复训闭环+评测闸门+告警出口+许可决策）→ M6（单盒多路网关）。细节与风险见 [docs/semantic-camera-design.md](docs/semantic-camera-design.md)。
