...
(function () {
  const PACKS = {
    ...
    'restricted-area': {
      ...
      // 危险行为定义（开放集，admin 主笔可修正）：慢脑 VLM 的判定依据，不是端侧分类器
      behaviors: [
        { id: 'perimeter-intrusion', name: '翻墙越栏',
          description: '人员攀爬围栏或围墙进入，而非经由大门通行',
          observable: '人体出现在围栏上方且越过',
          proposalTriggers: ['zone-hit', 'trajectory-discontinuity'],
          evaluation: 'mid-segment', cooldownMs: 15000, severity: 'high' },
        { id: 'loitering', name: '可疑滞留',
          description: '人员在敏感区域边缘长时间徘徊，无正常通行目的',
          observable: '同一个人在区域边缘反复出现，停留时间显著长于通行者',
          proposalTriggers: ['dwell'],
          evaluation: 'segment-end-only', cooldownMs: 30000, severity: 'medium' },
      ],
      arb: { budgetPerHour: 10, ttlMs: 15000 },
      selfTrain: null,
    },
